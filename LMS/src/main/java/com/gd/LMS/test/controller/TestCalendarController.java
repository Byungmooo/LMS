...
import com.gd.LMS.test.service.TestCalendarService;
...
public class TestCalendarController {
	@Autowired TestCalendarService testCalendarService;

	@GetMapping(value="testCalendar/{lectureNo}/attendance/attendanceByMonth/{currentYear}/{currentMonth}")
	public String cashbookByMonth(Model model, ServletRequest request,
		...
		// 해당 달의 출석 리스트
		List<AttendanceVo> attendanceList = testCalendarService.selectAttendanceList(map);
		// 전체일 출석률 
		double attendancePercent = testCalendarService.selectAttendancePercent(map);
