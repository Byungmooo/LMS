...
public class LectureNoticeController {
	...
	// 강의공지 목록 리스트
	@GetMapping({"/student/lectureNoticeList", "/student/lectureNoticeList"})
	public String lectureNoticeList(PagingVo vo, Model model, HttpSession session, Map<String, Object> map,
	...
	public String modifyLectureNotice(LectureNotice lectureNotice) {
		...
		if (count >= 1) {
			...
			return "redirect:/professor/lectureNoticeOne/" + lectureNotice.getLecNoticeNo();
		}
		return "redirect:/professor/modifyLectureNotice/" + lectureNotice.getLecNoticeNo();
	}
	...
	public String LecNoticeOne(@RequestParam(value = "lecNoticeNo") int lecNoticeNo) {
		...
		return "redirect:/professor/lectureNoticeOne/" + lecNoticeNo;
	}
