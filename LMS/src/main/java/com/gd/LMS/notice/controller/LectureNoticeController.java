...
public class LectureNoticeController {
	...
	public String TotalnNoticeList(PagingVo vo, Model model,
		...
		List<LectureNotice> list = lectureNoticeService.getLecNoticeList();
		model.addAttribute("list", list);

		int totalCount = lectureNoticeService.countBoard(keyword, searchType);
		...
		List<LectureNotice> list1 = lectureNoticeService.selectBoard(vo);
	...
	public String TotalNoticeOne(Model model, @PathVariable(value = "lecNoticeNo") int lecNoticeNo) {
		LectureNotice lecNotice = lectureNoticeService.getLecNoticeList(lecNoticeNo);
		lectureNoticeService.updateLecNoticeCount(lecNoticeNo);
	...
	public String addLecNotice(LectureNotice lectureNotice) {
		...
		int count = lectureNoticeService.addLecNotice(lectureNotice);
		if (count >= 1) {
	...
	public String updateLecNotice(LectureNotice lectureNotice) {
		int count = lectureNoticeService.updateLecNotice(lectureNotice);
		if (count >= 1) {
	...
	public String LecNoticeOne(@RequestParam(value = "lecNoticeNo") int lecNoticeNo) {
		int count = lectureNoticeService.deleteLecNotice(lecNoticeNo);
		if (count >= 1) {
