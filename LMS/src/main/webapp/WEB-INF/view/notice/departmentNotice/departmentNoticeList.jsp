...
<div class="container-xxl flex-grow-1 container-p-y">
				...
				<tbody>
					<c:forEach var="row" items="${list}">
						<tr>
							<td>${row.noticeNo}</td>
							<td><a
								href="${pageContext.request.contextPath}/member/totalNoticeOne?noticeNo=${row.noticeNo}">
									${row.noticeTitle} </a></td>
							<td>${row.writer}</td>
							<td>${row.views}</td>
	...
	<!-- 검색 -->
	<form
		action="${pageContext.request.contextPath}/member/departmentNoticeList"
		method="get">
		<div class="row">
			...
			<div class="col-sm-2 col-12">
				<c:if test="${sessionScope.memberType eq '직원'}">
					<a
						href="${pageContext.request.contextPath}/employee/addTotalNotice"
			...
			<ul class="pagination justify-content-center">
				<c:if test="${paging.prePage}">
					<li class="page-item prev"><a class="page-link"
						href="${pageContext.request.contextPath}/member/departmentNoticeList?currentPage=${paging.currentPage-1}
					...
					<c:choose>
						...
						<c:when test="${p != paging.currentPage }">
							<li class="page-item"><a class="page-link"
								href="${pageContext.request.contextPath}/member/departmentNoticeList?currentPage=${p}
