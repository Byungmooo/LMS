...
<!-- Main -->
				...
				<h3 class="card-header">시험지이름 : ${examSheet.examName}-${examSheet.examNo}</h3>
					<form id="examSheetForm">
						<div class="mt-2 mb-3">
							...
							<button type="button" onclick="examSheetUpdate();">수정</button>
						</div>
					</form>
		</div>
			...
			<div class="card mb-4">
				<h3 class="card-header">객관식 ${m.multipleNo}번 문제</h3>
					<div class="mt-2 mb-3">
						<label >※문제내용</label>
						<div>
							<input class="form-control" type="text" style="width:200px" placeholder="문제내용" value="${m.multipleContent}" id="multipleContent_${m.multipleNo }">
							<button type="button" onclick="multipleUpdate('${m.multipleNo}')">수정</button>
						</div>
							<div class="mt-2 mb-3">
									...
									<div>
										<c:forEach var = "ex" items="${m.exampleList}" varStatus="status">
												...
												<input class="form-control" type="text" style="width:200px" 
													placeholder="${status.count}번"  value="${ex}" id="exampleContent_${m.multipleNo }"> 
													
												<button type="button" onclick="exampleUpdate('${m.multipleNo }','${status.count}')">수정</button>
						...
						<div>
						<label>※정답</label>
						<label>1번<input type="radio" name="multipleAnswer_${m.multipleNo}" id="multipleAnswer" value="1"> </label>
						<label>2번<input type="radio" name="multipleAnswer_${m.multipleNo}" id="multipleAnswer" value="2"> </label>
						<label>3번<input type="radio" name="multipleAnswer_${m.multipleNo}" id="multipleAnswer" value="3"> </label>
						<label>4번<input type="radio" name="multipleAnswer_${m.multipleNo}" id="multipleAnswer" value="4"> </label>
						<button type="button" onclick="multipleAnswerUpdate('${m.multipleNo}')">수정</button>
						</div>
				...
				<h3 class="card-header">주관식 문제 ${s.subjectiveNo}번</h3>
						...
						<div class="mt-2 mb-3">
							...
							<input class="form-control" type="text" style="width:400px" 
									placeholder="문제"  value="${s.subjectiveContent }" id="subjectiveContent_${s.subjectiveNo}">
							<button type="button" onclick="subjectiveUpdate('${s.subjectiveNo}');">수정</button>
						</div>
						
						<div class="mt-2 mb-3">
							<label>정답</label>
							<input class="form-control" type="text" style="width:200px" 
									placeholder="정답"   value="${s.subjectiveAnswer }"  id="subjectiveAnswer_${s.subjectiveNo}">
									<button type="button" onclick="subAnswerUpdate('${s.subjectiveNo}');">수정</button>
			        	</div>
	...
	<script>
		...
		function multipleAnswerUpdate(multipleNo){
			...
			var multipleAnswer =$('input[name="multipleAnswer_'+multipleNo+'"]:checked').val();
			var examNo = $('#exam_no').val();
