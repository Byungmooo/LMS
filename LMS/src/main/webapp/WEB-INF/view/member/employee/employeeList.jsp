...
<div class="container-xxl flex-grow-1 container-p-y">
			...
			<li class="nav-item"><c:choose>
					...
					<c:when test="${memberType eq '직원'}">
						<a class="nav-link"
							...
							<i class="bx bx-user me-1"></i>승인된 직원 전체리스트
						</a>
					</c:when>
					<c:otherwise>
					</c:otherwise>
				...
				<tbody>
					<c:forEach var="e" items="${list}">
						<tr>
							<td>${e.memberId}</td>
							<td><a
								href="${pageContext.request.contextPath}/employee/employeeOne?employeeCode=${e.employeeCode}">${e.memberName}</a>
							</td>
	...
	<div class="row">
		...
		<div class="col-sm-7 col-12 text-center">
			<form
				action="${pageContext.request.contextPath}/employee/employeeList"
				method="get">
		...
		<div style="display: block; text-align: center;">
			<c:if test="${paging.prePage}">
				<a
					href="${pageContext.request.contextPath}/employee/employeeList?currentPage=${paging.currentPage-1}
			...
			<c:forEach begin="${paging.startPage}" end="${paging.endPage}"
				...
				<c:choose>
					...
					<c:when test="${p != paging.currentPage }">
						<a
							href="${pageContext.request.contextPath}/employee/employeeList?currentPage=${p}
